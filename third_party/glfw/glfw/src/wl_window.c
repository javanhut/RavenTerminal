                                    ...
                                    const _GLFWfbconfig* fbconfig)
{
    ...
    // Backport of GLFW 3.4's app_id: reuse the X11 class-name hint so Wayland
    // compositors (KWin, mutter) can match this window to its .desktop file.
    if (strlen(wndconfig->x11.className))
        window->wl.appId = _glfw_strdup(wndconfig->x11.className);
